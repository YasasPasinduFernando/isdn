...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_approval') {
    ...
    // include source_rdc_id so we can verify the transfer belongs to the current RDC
        $q = $pdo->prepare('SELECT approval_status, destination_rdc_id, source_rdc_id FROM stock_transfers WHERE transfer_id = ?');
        $q->execute([$transfer_id]);
        $row = $q->fetch(PDO::FETCH_ASSOC);
        if (!$row) json_error('Transfer not found.', 404);

        // Ensure this transfer is intended for this RDC
        if ((int)$row['source_rdc_id'] !== (int)$current_user['rdc_id']) json_error('Transfer does not belong to your RDC.', 403);
        ...
        // If approving, we must decrement stock at the source RDC for each item
        ...
        if ($new_status === 'APPROVED') {
            ...
            // Prepare statements to lock and update stock rows
            ...
            $sourceRdcId = (int)$row['source_rdc_id'];

            foreach ($items as $it) {
                $pid = (int)$it['product_id'];
                $reqQty = (int)$it['requested_quantity'];

                // Lock the product stock row for this RDC
                $selectStock->execute([$sourceRdcId, $pid]);
                $stockRow = $selectStock->fetch(PDO::FETCH_ASSOC);
                if (!$stockRow) {
                    // No stock record for this product at source RDC
                    $pdo->rollBack();
                    json_error("Stock record not found for product_id {$pid} at source RDC.", 400);
                }

                $available = (int)($stockRow['available_quantity'] ?? 0);
                if ($available < $reqQty) {
                    // Insufficient stock to approve
                    $pdo->rollBack();
                    json_error("Insufficient stock for product_id {$pid}. Available: {$available}, required: {$reqQty}", 409);
                }

                $newAvail = $available - $reqQty;
                $updateStock->execute([$newAvail, $stockRow['id']]);
            }
        }
    ...
    json_success('Decision recorded successfully.', ['new_status' => $new_status, 'approved_by' => $current_user['name'], 'approval_date' => date('Y-m-d H:i:s')]);
