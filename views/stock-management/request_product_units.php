            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4">
                        ...
                        <div id="modal-requested-time" class="text-xs font-bold text-orange-900"></div>
                    </div>
...
<script>
    ...
    function openTransferModal(index) {
        ...
        document.getElementById('modal-requested-date').textContent = new Date(transfer.requested_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        document.getElementById('modal-requested-time').textContent = new Date(transfer.requested_date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
        ...
        transfer.items.forEach(item => {
            ...
            <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition">
                ...
                <div class="mt-3 received-qty-row hidden">
                    <label class="block text-xs text-gray-600 mb-1">Received Quantity</label>
                    <input type="number" min="0" max="${item.requested_quantity}" value="${item.requested_quantity}" class="w-32 px-3 py-2 border border-gray-300 rounded received-qty-input" data-product-id="${item.product_id}">
                    <div class="text-xs text-gray-500 mt-1">Enter the actual quantity received for this product (max ${item.requested_quantity}).</div>
                </div>
        ...
        // After building products, ensure received qty inputs are shown only for APPROVED transfers
        const showReceivedInputs = transfer.status === 'APPROVED';
        productsList.querySelectorAll('.received-qty-row').forEach(row => {
            if (showReceivedInputs) row.classList.remove('hidden'); else row.classList.add('hidden');
        });
    ...
    document.getElementById('submit-status-btn')?.addEventListener('click', async function(e) {
        ...
        // If marking as RECEIVED, collect received quantities per item
        if (selectedNewStatus === 'RECEIVED') {
            const received = {};
            document.querySelectorAll('.received-qty-input').forEach(input => {
                const pid = input.getAttribute('data-product-id');
                const val = parseInt(input.value || '0', 10);
                received[pid] = isNaN(val) ? 0 : val;
            });
            form.append('received_items', JSON.stringify(received));
        }
