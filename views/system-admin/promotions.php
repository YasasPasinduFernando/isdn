    ...
    try {
        ...
        if ($act === 'create' || $act === 'update') {
            ...
            if (empty($data['name']) || empty($data['product_id']) || empty($data['start_date']) || empty($data['end_date'])) {
                ...
                redirect('/index.php?page=system-admin-promotions&action=' . ($act === 'update' ? 'edit&id=' . $_POST['promo_id'] : 'add'));
            }
            ...
            if ($act === 'create') {
                ...
                $admin->updatePromotion($pid, $data);
                $admin->logAction($_SESSION['user_id'], 'UPDATE', 'promotion', $pid, "Updated promotion: " . $data['name']);
                flash_message('Promotion updated successfully!', 'success');
            }
            redirect('/index.php?page=system-admin-promotions');

        } elseif ($act === 'toggle' && !empty($_POST['promo_id'])) {
            $pid = (int) $_POST['promo_id'];
            $admin->togglePromotionActive($pid);
            $admin->logAction($_SESSION['user_id'], 'TOGGLE', 'promotion', $pid, "Toggled promotion active status");
            flash_message('Promotion status updated.', 'success');
            redirect('/index.php?page=system-admin-promotions');

        } elseif ($act === 'delete' && !empty($_POST['promo_id'])) {
            $pid = (int) $_POST['promo_id'];
            $p = $admin->getPromotionById($pid);
            $admin->deletePromotion($pid);
            $admin->logAction($_SESSION['user_id'], 'DELETE', 'promotion', $pid, "Deleted promotion: " . ($p['name'] ?? '#' . $pid));
            flash_message('Promotion deleted.', 'success');
            redirect('/index.php?page=system-admin-promotions');
        }
    } catch (Exception $e) {
...
if ($action === 'add' || $action === 'edit') {
    ...
    if ($action === 'edit' && !empty($_GET['id'])) {
        $editPromo = $admin->getPromotionById((int) $_GET['id']);
        if (!$editPromo) {
            flash_message('Promotion not found.', 'error');
            redirect('/index.php?page=system-admin-promotions');
        }
    }
...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-8">
            <a href="<?php echo BASE_PATH; ?>/index.php?page=system-admin-promotions"
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800 font-['Outfit']">
                    <?php echo $editPromo ? 'Edit Promotion' : 'Add New Promotion'; ?>
                </h1>
                <p class="text-sm text-gray-500">
                    <?php echo $editPromo ? 'Update promotion #' . $editPromo['id'] : 'Create a new promotional offer'; ?>
                </p>
            </div>
        </div>

        <form method="POST" action="<?php echo BASE_PATH; ?>/index.php?page=system-admin-promotions"
              class="glass-panel rounded-3xl p-6 sm:p-8 space-y-6">
            <input type="hidden" name="form_action" value="<?php echo $editPromo ? 'update' : 'create'; ?>">
            <?php if ($editPromo): ?>
                <input type="hidden" name="promo_id" value="<?php echo $editPromo['id']; ?>">
            <?php endif; ?>
            ...
            <div>
                ...
                <input type="text" name="name" required
                       value="<?php echo htmlspecialchars($editPromo['name'] ?? ''); ?>"
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 transition shadow-sm"
                          placeholder="Brief description of this promotion..."><?php echo htmlspecialchars($editPromo['description'] ?? ''); ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <select name="product_id" required
                        ...
                        <?php foreach ($products as $prod): ?>
                            <option value="<?php echo $prod['product_id']; ?>"
                                <?php echo (($editPromo['product_id'] ?? '') == $prod['product_id']) ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="number" name="product_count" required min="1"
                           value="<?php echo htmlspecialchars($editPromo['product_count'] ?? '1'); ?>"
                           class="w-full border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 transition shadow-sm"
                           placeholder="e.g. 10">
                </div>
                <div>
                    ...
                    <input type="number" name="discount_percentage" required step="0.01" min="0.01" max="100"
                           value="<?php echo htmlspecialchars($editPromo['discount_percentage'] ?? ''); ?>"
                           class="w-full border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 transition shadow-sm"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="start_date" required
                           value="<?php echo htmlspecialchars($editPromo['start_date'] ?? ''); ?>"
                           class="w-full border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 transition shadow-sm">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">
                    ...
                    <input type="date" name="end_date" required
                           value="<?php echo htmlspecialchars($editPromo['end_date'] ?? ''); ?>"
                           class="w-full border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 transition shadow-sm">
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer"
                        <?php echo (!$editPromo || ($editPromo['is_active'] ?? 1)) ? 'checked' : ''; ?>>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200/50">
                ...
                <button type="submit"
                        class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold text-sm shadow-lg shadow-orange-200/50 hover:scale-[1.02] transition flex items-center gap-2">
                    <span class="material-symbols-rounded text-base"><?php echo $editPromo ? 'save' : 'add_box'; ?></span>
                    <?php echo $editPromo ? 'Save Changes' : 'Create Promotion'; ?>
                </button>
            </div>
        </form>
    </div>
...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8">
            <div class="flex items-center gap-3">
                <a href="<?php echo BASE_PATH; ?>/index.php?page=system-admin-dashboard"
                   class="w-10 h-10 rounded-xl bg-white/50 border border-white/60 flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-white/70 transition">
                    <span class="material-symbols-rounded">arrow_back</span>
                </a>
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 font-['Outfit']">Promotion Management</h1>
                    <p class="text-sm text-gray-500"><?php echo number_format($totalPromos); ?> promotions found</p>
                </div>
            </div>
            <a href="<?php echo BASE_PATH; ?>/index.php?page=system-admin-promotions&action=add"
               class="mt-4 md:mt-0 px-5 py-2.5 rounded-full bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold text-sm shadow-lg shadow-orange-200/50 hover:scale-[1.02] transition flex items-center gap-2 w-fit">
        ...
        <form method="GET" class="glass-card rounded-2xl p-5 mb-6 flex flex-wrap gap-4 items-end">
            <input type="hidden" name="page" value="system-admin-promotions">
            ...
            <div>
                ...
                <select name="status"
                        class="border border-white/40 bg-white/30 backdrop-blur-sm rounded-xl px-3 py-2 text-sm min-w-[160px] focus:ring-2 focus:ring-teal-500 transition shadow-sm">
            ...
            <button type="submit"
                    class="px-5 py-2 rounded-xl bg-gradient-to-r from-teal-500 to-teal-600 text-white font-bold text-sm shadow-lg hover:scale-[1.02] transition flex items-center gap-1.5">
        ...
        <div class="glass-panel rounded-3xl p-6 sm:p-8 mb-6">
            <?php if (empty($promotions)): ?>
                <div class="text-center py-16">
                    ...
                    <p class="text-gray-400 text-sm mt-1">Create your first promotion to get started.</p>
                </div>
            <?php else: ?>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100/50">
                            ...
                            <tr class="hover:bg-white/30 transition group">
                                ...
                                <td class="py-3 pr-3 text-center">
                                    <span class="text-lg font-bold text-orange-600"><?php echo number_format($promo['discount_percentage'], 1); ?>%</span>
                                </td>
                                <td class="py-3 pr-3">
                                ...
                                <td class="py-3 text-right">
                                    <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition">
                                        <a href="<?php echo BASE_PATH; ?>/index.php?page=system-admin-promotions&action=edit&id=<?php echo $promo['id']; ?>"
                                           class="w-8 h-8 rounded-lg bg-blue-100/50 text-blue-600 flex items-center justify-center hover:bg-blue-200 transition"
                                           title="Edit">
                                            <span class="material-symbols-rounded text-base">edit</span>
                                        </a>
                                        <form method="POST" class="inline" onsubmit="return confirm('Toggle promotion status?')">
                                            <input type="hidden" name="form_action" value="toggle">
                                            <input type="hidden" name="promo_id" value="<?php echo $promo['id']; ?>">
                                            <button class="w-8 h-8 rounded-lg <?php echo $isActive ? 'bg-yellow-100/50 text-yellow-600 hover:bg-yellow-200' : 'bg-green-100/50 text-green-600 hover:bg-green-200'; ?> flex items-center justify-center transition"
                                                    title="<?php echo $isActive ? 'Disable' : 'Enable'; ?>">
                                                <span class="material-symbols-rounded text-base"><?php echo $isActive ? 'visibility_off' : 'visibility'; ?></span>
                                            </button>
                                        </form>
                                        <form method="POST" class="inline" onsubmit="return confirm('Delete this promotion permanently?')">
                                            <input type="hidden" name="form_action" value="delete">
                                            <input type="hidden" name="promo_id" value="<?php echo $promo['id']; ?>">
                                            <button class="w-8 h-8 rounded-lg bg-red-100/50 text-red-500 flex items-center justify-center hover:bg-red-200 transition"
                                                    title="Delete">
        ...
        <div class="flex items-center justify-center gap-2">
            ...
            ?>
                <a href="<?php echo BASE_PATH; ?>/index.php?<?php echo $qs; ?>"
                   class="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold transition <?php echo $isActivePg ? 'bg-teal-500 text-white shadow-lg shadow-teal-200/50' : 'bg-white/50 text-gray-600 hover:bg-white/70 border border-white/60'; ?>">
                    <?php echo $i; ?>
                </a>
            <?php endfor; ?>
        </div>
